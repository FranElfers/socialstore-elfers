...
export default function LoadProduct() {
	...
	const titleRef = {ref: useRef(), id: 'title', title: 'Nombre del producto', type: 'text'}
	const descriptionRef = {ref: useRef(), id: 'description', title: 'Descripcion', type: 'text'}
	const pictureUrlRef = {ref: useRef(), id: 'pictureUrl', title: 'Imagen (url)', type: 'url'}
	const stockRef = {ref: useRef(), id: 'stock', title: 'Stock', type: 'number'}
	const priceRef = {ref: useRef(), id: 'price', title: 'Precio', type: 'number'}
	...
	const CategorySelector = () => {
		...
		const handleChange = ({target: {value: input}}) => {
			...
			if (input) {
				...
				const found = categories.find(cat => cat.startsWith(input.toLowerCase())) || ''

				const capitalizedInput = input.charAt(0) === input.charAt(0).toUpperCase()

				setSuggestion(
					// La sugerencia se adhiere al estado de la primera letra del input
					capitalizedInput ? found.charAt(0).toUpperCase() + found.slice(1) : found
				)
		...
		return <>
			<form autoComplete="off" onSubmit={e=>searchCategory(suggestion.toLowerCase(),e)} className="category-form">
		...
		<div className="🛒">
			<div className="total" style={{height:'auto'}}>
				...
				<UserForm 
					...
					preFields={[titleRef, descriptionRef, pictureUrlRef, stockRef, priceRef]} 
				/>
